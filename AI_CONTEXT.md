...
当前开发优先级以 [ROADMAP.md](./ROADMAP.md) 为准。

近期不优先追求组件数量或完整生产级能力。当前顺序是：先做一轮有限范围的 UI 基线升级，再稳定 MVP 核心闭环：

```text
UI 基线升级 -> 数据接入 -> 字段识别 -> 图表配置 -> 画布排版 -> 保存恢复 -> 导出运行
```

UI 基线升级采用“编辑器参考方案 1 + 方案 3，任务大厅参考方案 4”的方向，参考图统一保存在 `docs/ui-concepts/`。这轮只做产品外壳、布局层级、设计变量和关键状态，不大规模重构业务逻辑。随后第一功能阶段的最高优先级是统一 `globalData` 数据契约，解决数组型 mock 数据、对象型全局数据、字段推导、图表绑定和导出代码之间的不一致。只有这条黄金路径稳定后，再进入编辑器可用性、组件体系和生产化能力建设。
